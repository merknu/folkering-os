...
def make_weights(cfg: dict, seed: int = 42) -> dict:
    ...
    for li in range(L):
        ...
        # Qwen2.5 has biases on q/k/v but NOT on o.
        out[f"{prefix}.self_attn.q_proj.bias"] = np.zeros(H,   dtype=np.float32)
        out[f"{prefix}.self_attn.k_proj.bias"] = np.zeros(Hkv, dtype=np.float32)
        out[f"{prefix}.self_attn.v_proj.bias"] = np.zeros(Hkv, dtype=np.float32)
