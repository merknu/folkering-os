...
pub(super) fn run_refactor_step(draug: &mut compositor::draug::DraugDaemon, now_ms: u64) {
    ...
    loop {
        ...
        let llm_result = match llm_generate(compositor::draug::model_for_level(level), &prompt, &mut llm_buf) {
            Some(s) => s,
            None => {
...
`fn main`, no attributes — just the single `fib` function.";
    ...
    let mut llm_buf = [0u8; 8192];
    let llm_result =
        match llm_generate(compositor::draug::model_for_level(1), prompt, &mut llm_buf) {
            Some(s) => s,
            None => {
...
pub(crate) fn extract_rust_code_block(raw: &str) -> alloc::string::String {
    // Primary: find "```rust"
