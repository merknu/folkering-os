...
fn cmd_ask(full_cmd: &str) {
    ...
    // Check inference server status first
    ...
    // Create shared memory for the query
    ...
    let handle = match shmem_create(shmem_size) {
        Ok(h) => h,
        Err(_) => {
            println!("[AI] Failed to allocate shared memory");
            return;
        }
    };

    // Grant to inference server
