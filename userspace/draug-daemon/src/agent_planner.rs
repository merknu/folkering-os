...
use crate::draug::{TaskPlan, PlanStep};
use crate::knowledge_hunt::{extract_rust_code_block, write_dec};
...
pub fn plan_task(task_id: &str, task_desc: &str) -> Option<TaskPlan> {
    ...
    let result = llm_generate(crate::draug::PLANNER_MODEL, &prompt, &mut llm_buf)?;
    if result.status != 0 {
...
pub fn execute_next_step(plan: &mut TaskPlan) -> bool {
    ...
    loop {
        ...
        let llm_result = match llm_generate(crate::draug::EXECUTOR_MODEL, &effective_prompt, &mut llm_buf) {
            Some(s) => s,
            None => {
